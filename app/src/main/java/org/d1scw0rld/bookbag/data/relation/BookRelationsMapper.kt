...
object BookRelationsMapper {
     ...
     */
    fun mapBooksToParents(allBooks: List<BookWithFields>, iOrder: Int): ArrayList<ParentResult> {
        ...
        when (iOrder) {
            DBAdapter.SRT_TTL -> {
                ...
                for ((parent, list) in grouped.entries.sortedBy { it.key }) {
                    ...
                    for (relation in sortedList) {
                        val authors = relation.fields.filter { it.typeId == DBAdapter.FLD_AUTHOR }.joinToString(", ") { it.name }
            ...
            DBAdapter.SRT_AUT -> {
                ...
                for (relation in allBooks) {
                    val authors = relation.fields.filter { it.typeId == DBAdapter.FLD_AUTHOR }
                    if (authors.isEmpty()) {
            ...
            DBAdapter.SRT_RD_AUT -> {
                // Read books grouped by Read Date (Year)
                val filtered = allBooks.filter { relation ->
                    val read = relation.fields.firstOrNull { it.typeId == DBAdapter.FLD_READ }?.name.orEmpty()
            ...
            DBAdapter.SRT_NOT_RD_AUT -> {
                ...
                val grouped = filtered.groupBy { relation ->
                    relation.fields.firstOrNull { it.typeId == DBAdapter.FLD_FORMAT }?.name ?: MISSING
                }
            ...
            DBAdapter.SRT_PBL_AUT -> {
                // Grouped by Publisher
                val grouped = allBooks.groupBy { relation ->
                    relation.fields.firstOrNull { it.typeId == DBAdapter.FLD_PUBLISHER }?.name ?: MISSING
                }
            ...
            DBAdapter.SRT_LND_TTL -> {
                // Grouped by Loaned To, sorted by Title
                val grouped = allBooks.groupBy { relation ->
                    relation.fields.firstOrNull { it.typeId == DBAdapter.FLD_LOANED_TO }?.name ?: MISSING
                }
